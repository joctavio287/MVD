...
if (api_key == "") {
  stop("No se encontró NEWSAPI_KEY. Revisá tu archivo .Renviron")
} else {
  cat("API Key cargada")
}
```
...
En base a esta tabla ya podemos realizar exactamente el mismo análisis que la vez pasada ([les queda como tarea]{.underline}). Ahora vamos a *LogOddsRatio*.
...
ggplot(top_palabras, aes(x = log_odds_plot, y = lemma, color = fuente)) +
