...
noticias_gv <- tibble(
  titulo = titulos[(length(titulos)-n_noticias):n_noticias],
  url = urls[(length(urls)-n_noticias):n_noticias],
  resumen = resumenes[(length(urls)-n_noticias):n_noticias],
  fecha_raw = fechas[(length(urls)-n_noticias):n_noticias] # raw porque es un character, por ahora
)
...
scrapear_gv <- function(url, first_page=FALSE) {
  ...
  return(
    tibble(
      titulo = titulos[(length(titulos)-n):n],
      url = urls[(length(urls)-n):n],
      resumen = resumenes[(length(urls)-n):n],
      fecha_raw = fechas[(length(urls)-n):n]
    )
  )} else {
    tibble(
      titulo = titulos[1:n],
...
Este gráfico muestra que, en relación a la migración e inmigración, entre los términos más relevantes de noticias mundiales, se destacan "China", "India", "refugiados", etc.
...
En términos de extremos, tendremos que valores muy a la derecha y abajo serán, generalmente, palabras asociadas al lenguaje (suelen saltar *stopwords* que no filtramos correctamente).

Mientras que para valores muy a la izquierda y arriba habrá palabras que aparecen en menor medida en el corpus total, pero igual destacan.
