...
En este caso, la celda de abajo separa el texto en palabras. Sin embargo, no vamos a seguir por este camino ya que la el proceso de lematización que hicimos arriba ya identificó las palabras de cada noticia.
...
Las *stop words* son palabras muy comunes que aparecen en casi todos los textos pero que aportan poca carga semántica. En español, estas incluyen artículos ("el", "la"), preposiciones ("de", "en"), pronombres, etc.
...
Hacemos un `antijoin` entre la BD y las `stop_word`. El objetivo es quedarnos únicamente con las palabras que no estén en `stop_words`.
...
ggplot(top_terminos, aes(x = tfidf_promedio, 
  ...
  geom_col(aes(fill = n_documentos), alpha = 0.8) +
  # geom_col(alpha = 0.8) +
  labs(
...
ggplot(top_frecuentes, aes(x = n, y = reorder(lemma, n))) +
  ...
  # geom_point(size = 4, aes(color = n_documentos)) +
  labs(
...
Se observa que si tomamos un valor grande recuperamos palabras asociadas al lenguaje común del corpus ("país", "hacer", "persona", "año", etc.); con valores bajos palabras salientes ("ucraniano", "ruso") y con valores medianos (\~10) tópicos generales ("visa", "estudiante", "mujer", "refugiado").
