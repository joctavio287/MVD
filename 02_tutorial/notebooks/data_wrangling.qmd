...
```{r}
planes |> select(year) |> unique() |> arrange(year) # ERROR esto no nos sirve porque no los mismos aviones que vuelos_retrasadaos_ua
...
**Disclaimer**: no soy economista ni aficionado de estos temas, pero sí me parece importante aclarar que cuando uno se enfrenta a un fenómeno que no conoce es importante estudiar un poco de qué se trata para poder entender qué queremos hacer.
...
Descargamos los datos directamente desde sus URLs oficiales ([PBG](https://catalogo.datos.gba.gob.ar/dataset/pbg) y [Nación](https://www.argentina.gob.ar/economia/politicaeconomica/regionalysectorial/informesproductivos/datasets)).
...
nacion |> distinct(indicador) |> arrange(indicador) |> head()
```
...
Vamos a hacer una limpieza de PBA. En la página web que está más arriba también vamos a encontrar un catálogo para la base de datos.

Digeramos el comando que vamos a usar a continuación para eliminar datos faltantes (`na`): **1)** **`is.na()`** identifica los valores faltantes. **2)** El punto (**`.`**) actúa como un marcador de posición (*placeholder*) que representa a la columna que se está evaluando en cada paso. **3)** Aplicamos la negación (**`!`**) para que los datos reales pasen a ser `TRUE` y los faltantes `FALSE`. **4)** Luego, usamos **`any()`** para verificar si existe al menos un valor verdadero (es decir, un dato útil) en toda la columna. Finalmente, combinamos **`where()`** y la tilde (**`~`**) para aplicar esta función de filtrado a todas las columnas del dataset, conservando solo aquellas que no estén totalmente vacías.
...
Busquemos la variable que nos interesa.
...
Agregamos los valores de la base de datos de PBA a valores constantes, igual que en el otro caso.
...
```{r}
# pba_agg es el que calculamos con la base de PBA
...
# pbg_pba es el que calculamos con la base de nacion
proporcion_pgb_ba_por_nacion = pbg_pba |> 
  left_join(pbg_nacion_17, by="year") |> 
  mutate(proporcion_ba = 100 * (valor / valor_nacion_17)) |> 
  select(year, proporcion_ba)

head(proporcion_pgb_ba)
head(proporcion_pgb_ba_por_nacion, 10)
```
...
comparativa_proporciones <- proporcion_pgb_ba |> 
  ...
  left_join(proporcion_pgb_ba_por_nacion, by = "year") |> 
  rename(prop_desde_nacion_file = proporcion_ba) |> 
...
Al contrastar las dos series, observamos inconsistencias y oscilaciones bruscas en la serie de Nación. Esto se debe a un problema de **integridad referencial entre fuentes**: el denominador (Nación) no siempre contiene la misma cantidad de provincias reportadas año a año, lo que genera saltos artificiales en la proporción.

```{r}
head(pbg_nacion_17)
pbg_nacion_17 |> select(provincias_contabilizadas) |> unique()

```

es esperable que si cambiamos la cantidad de provincias involucradas en el promedio, la proporción asignaba a BA varíe. De hecho, la línea azul decrece constantemente porque el número de provincias involucradas en el promedio aumenta con los años.

```{r}
pbg_nacion_17 |> select(year, provincias_contabilizadas)
```

Además, estamos mezclando el PBG (que incluye impuestos) del archivo de Nación con la suma de sectores del archivo provincial que no los incluye.
