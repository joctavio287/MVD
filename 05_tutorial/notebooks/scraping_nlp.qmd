...
1.  **Primera parte**: vamos a aprender a hacer scraping de noticias de ~~la ONU sobre Paz y Seguridad~~ utilizando `rvest` y `httr2`.
...
3.  **Tercera parte**: aplicaremos técnicas de **NLP** (tokenización, eliminación de *stop words* y cálculo de TF-IDF) para identificar los términos más relevantes en las noticias.
4.  **Cuarta parte**: visualizaremos los resultados con `ggplot2` para interpretar los hallazgos.

Para hacer este estudio nos vamos a guiar por la siguiente pregunta de investigación:

> *¿Cuáles son los términos más distintivos que caracterizan las noticias sobre paz y seguridad de la ONU?*
...
Muchas veces los datos que necesitamos no están disponibles en formatos descargables como CSV o Excel, como venimos haciendo hasta ahora, sino que se encuentran en páginas web. El web *scraping* nos permite automatizar la extracción de esta información.

Antes de hacer *scraping*, **siempre** debemos:

-   Revisar el archivo `robots.txt` del sitio
-   Respetar los Términos de Servicio
-   No sobrecargar los servidores (usar pausas entre *requests*)
...
# paths = "https://news.un.org/es/news/topic/peace-and-security", --> FALSE, una pena
```
...
Primero, vamos a cargar la página principal de noticias sobre Argentina en un diario español. Para eso, vamos a usar la librería `rvest` que es el estándar de R para hacer *scraping*. Algunos comandos útiles son
...
pagina_html |> write_rds(file.path(data_dir, "pagina_noticias_gv.rds"))

# Guardamos en html para abrir en el navegador si queremos
write_html(pagina_html, file = file.path(data_dir, "pagina_noticias_gv.rds.html"))
```
...
Cuando estamos haciendo un análisis exploratorio, ir y venir al navegador **es buena práctica para entender cómo se estructura visualmente la información**. Fíjense que aparecen más fechas urls y títulos (29) que resúmenes, esto se debe a que las cuatro primeras noticias aparecen únicamente como títulos.
...
n_noticias <- min(length(titulos), length(urls), length(resumenes), length(fechas))

# Creamos un tibble con los datos extraídos (descartamos aquellas que no tienen copete)
noticias_gv <- tibble(
  titulo = titulos[1:n_noticias],
  url = urls[1:n_noticias],
  resumen = resumenes[1:n_noticias],
  fecha_raw = fechas[1:n_noticias] # raw porque es un character, por ahora
)
...
scrapear_gv <- function(url) {
  ...
  n <- min(length(titulos), length(urls), length(resumenes), length(fechas))
  
  return(
    tibble(
      titulo = titulos[1:n],
      url = urls[1:n],
      resumen = resumenes[1:n],
      fecha_raw = fechas[1:n]
    )
  )
}
...
scrapear_gv("https://es.globalvoices.org/category/topics/migration-immigration/")
...
scrapear_n_gv <- function(n) {
  ...
  noticias_pagina <- scrapear_gv("https://es.globalvoices.org/category/topics/migration-immigration/")
  ...
  for (i in 2:n) {
    ...
    mensaje <- paste("\nPágina", i, "scrapeada correctamente. Total noticias hasta ahora:", nrow(noticias_pagina), "de", n -1)
...
# Guardamos este objeto en disco
attr(noticias_gv, "fecha_descarga") <- Sys.time()
write_rds(noticias_gv, file.path(data_dir, "noticias_gv.rds"))
```

### Función que permita extraer el cuerpo del noticia

Genial, ahora sería utila, para cada noticia, extraer el texto asociado. Esto va a enriquecer nuestra base de datos
...
[**ATENCIÓN**]{.underline}**:** si el `n` anterior era muy grande vamos a estar ejecutando la función por cada una de las `n*#de noticias en cada página`. Esto no significa que el archivo sea especialmente pesado, pero sí que el tiempo de ejecución va a ser largo.

Por ejemplo, para `n=3` y teniendo en cuenta que en cada página hay 75 noticias, vamos a leer el texto de 225 notas. Esto tardará \~4 minutos, dado que por cada noticia tenemos un `Sys.sleep(1).`
...
attr(cuerpos_noticias_gv, "fecha_descarga") <- noticias_gv |>
  attr("fecha_descarga") 
...
Ahora sí, metemos todo en la misma tabla, por simplicidad.
...
2.  **Eliminación de *stop words***: quitar palabras muy comunes que no aportan significado ("el", "la", "de", etc.).
3.  **TF-IDF**: identificar qué términos son más distintivos dentro del corpus de noticias.
...
# Notar que conviertre el tibble a formato long, donde cada fila es un token (palabra) de una noticia específica.
...
stop_words <- tibble(palabra = c(stop_es, stop_en))

# Verificamos algunas stop words
head(stop_words, 20)
```

Hacemos un `antijoin` entre la BD y las `stop_word_es`. El objetivo es quedarnos únicamente con las palabras que no estén en `stop_word_es`.

```{r}
# Contamos las palabras antes
numero_de_palabras <- nrow(noticias_tokens)

# Eliminamos las stop words
noticias_tokens <- noticias_tokens |>
  anti_join(stop_words, by = "palabra") |>
  # También eliminamos números y palabras de una sola letra
  filter(
    !str_detect(palabra, "^\\d+$"), 
    nchar(palabra) > 2               # Tienen más de 2 caracteres
  )
...
cat("Tokens después de eliminar stop words:", nrow(noticias_tokens), "\n")
cat(
  "Reducción relativa porcentual:", 
  round(
    100 * ((numero_de_palabras - nrow(noticias_tokens)) / numero_de_palabras),
    1
  ), 
  "%\n"
)
...
```{r}
palabras_frecuentes <- noticias_tokens |>
...
$$es decir, el número de veces que aparece el término $t$ en el documento $d$ ($N(t,d)$), dividido las apariciones totales de todos los términos en el documento $d$ ($sum_t N(t,d)$). El resultado se puede interpretar como una frecuencia normalizada (probabilidad).
...
En este sentido, notemos que cuando $D = D_{t}$ tenemos que el $IDF(t)=0$ lo cual tiene sentido porque se trata de un término que aparece en todos los documentos, puede ser redundante. Ahora, si $D_t<<D$ el logaritmo se hace mucho más grande que 1. Por ejemplo, si una palabra $\bar{t}$ está muy repetida en 1 solo documento sobre un total de 1000 documentos, tendremos que $IDF(\bar{t})=3)$.

![Logaritmo base diez cerca de uno.](log10.png){fig-align="center" width="313"}

Por lo tanto, la idea es darle más peso a palabras que:
...
noticias_tfidf <- noticias_tokens |>
  count(id, titulo, palabra) |>  # Contamos palabras por documento
  bind_tf_idf(palabra, id, n)     # Calculamos TF-IDF
...
Los valores de *TF-IDF* nos dicen qué palabras son más "distintivas" de cada noticia. Su fórmula es el producto de las dos métricas. Es importante notar que esta métrica es función de $t$ y $d$, ya que combina una métrica global con otra local.
...
# Veamos las primeras 3 noticias
top_palabras_por_noticia |>
  select(id, titulo, palabra, tf_idf) |> 
  arrange(id, desc(tf_idf))
  # arrange(desc(tf_idf))
```
...
terminos_distintivos <- noticias_tfidf |>
  ...
  filter(n_documentos >= 10) # VARIAR EN CLASE: es interesarte notar qué información recuperamos en cada caso
...
Si dejamos un valor bajo, los términos que aparecerán más arriba tendrán pocos documentos, ya que esto les dará un IDF más alto. Esto puede ser útil, pero no representar al corpus completo. Podría ser que hubo una noticia muy especifica sobre un tema que nunca más se volvió a hablar.

En cambio, si tomamos una valor moderado (en relación al total de documentos) los valores que obtendremos serán más representativos del corpus completo, pero perderán IDF.

En el extremo, si tomamos valores muy grandes, el IDF se volverá muy bajo y pasaremos a tener *stopwords* o términos que tienen poca importancia porque se pierde la relevancia del documento.
...
Visualizamos los términos más distintivos del corpus. Para esto optamos por el enfoque intermedio (`n_docs=10`), es decir, queremos encontrar tópicos relevantes.
...
ggplot(top_terminos, aes(x = tfidf_promedio, 
                         y = reorder(palabra, tfidf_promedio))) +
  geom_col(fill = "#4292C6", alpha = 0.8) +
  labs(
    title = "Top 20 Términos Más Distintivos",
    subtitle = "Noticias de Paz y Seguridad - ONU",
    x = "TF-IDF Promedio",
    y = NULL,
    caption = "Fuente: news.un.org | Análisis con tidytext"
  ) +
...
Nos enteramos que, en relación a la migración e inmigración, entre los términos más relevantes de noticias mundiales, se destacan "China", "India", "refugiados", etc.

### Palabras por frecuencias

Podemos crear una visualización alternativa mostrando las palabras más frecuentes:
...
ggplot(top_frecuentes, aes(x = n, y = reorder(palabra, n))) +
  geom_segment(aes(x = 0, xend = n, 
                   y = reorder(palabra, n), yend = reorder(palabra, n)),
               color = "gray70") +
...
comparacion <- palabras_frecuentes |>
  left_join(
    terminos_distintivos |> select(palabra, tfidf_promedio),
    by = "palabra"
  ) |>
  filter(!is.na(tfidf_promedio)) |>
  slice_max(n, n = 80) # Tomamos las 80 palabras más frecuentes para comparar

# Gráfico de dispersión
ggplot(comparacion, aes(x = n, y = tfidf_promedio)) +
  geom_point(size = 3, alpha = 0.6, color = "#08519C") +
  ...
  labs(
    title = "Frecuencia vs. TF-IDF",
    subtitle = "¿Las palabras más frecuentes son las más distintivas?",
...
[**Interpretación**]{.underline}: este gráfico es muy útil porque nos ayuda a interpretar las métricas.

En términos de extremos, tendremos que valores muy a la derecha y abajo serán, generalmente, palabras asociadas al lenguaje (suelen saltar *stopwords* que no filtramos correctamente). Mientras que valores muy a la izquierda y arriba serán palabras que aparecen poco en el corpus total, pero igual destacan.

### Términos distintivos por noticia individual

Finalmente, podemos visualizar los términos más distintivos de algunas noticias seleccionadas:
...
noticias_seleccionadas <- top_palabras_por_noticia |>
  ...
  ungroup() |>
  mutate(
    titulo_corto = str_trunc(titulo, width = 50)
  )

ggplot(noticias_seleccionadas, 
       aes(x = tf_idf, y = reorder_within(palabra, tf_idf, id))) +
